/**
 * TradingView 指标数据采集客户端（K线增强版，独立于 tv-indicator.js）
 *
 * 在 tv-indicator.js 的基础上增加K线数据采集（原文件保持不变，本文件独立运行）：
 *   - cur_kline    主周期（当前订阅周期）已收盘K线 → 直接复用主 chart 的 periods，不额外开订阅
 *   - kline_<tf>m  辅助周期已收盘K线（默认 1 分钟）→ 同一 WebSocket client 上额外开一条 chart 订阅
 *
 * 写入 data 字段的 JSON 示例：
 *   { "m15_duo": 2482.0, "m10_kong": 2478.0, ...,
 *     "cur_kline": [{ "time": 1788267900, "open": 2489.0, "high": 2491.0, "low": 2480.0, "close": 2489.5, "volume": 1234.56 }, ...],
 *     "kline_1m":  [{ "time": 1788267930, "open": 2491.5, "high": 2492.5, "low": 2491.0, "close": 2492.0, "volume": 56.78 }, ...] }
 *
 * 【辅助K线默认配置】（.env 为多客户端共用，辅助K线仅与本客户端相关，故不放 .env）
 *   默认值在下方常量定义，命令行未传参时采用；传参则覆盖：
 *   - DEFAULT_KLINE_TIMEFRAME = '1' → 辅助周期 1 分钟，写入键名 kline_1m
 *   - DEFAULT_KLINE_COUNT     = 3   → 每次写入 3 根已收盘K线（同时作用于 cur_kline）
 *   命令行覆盖：
 *   - -t <分钟>  辅助周期，如 -t 5 → kline_5m
 *   - -n <根数>  K线条数，如 -n 5
 *   示例：node clients/tv-indicator-kline.js -k <api_key> -i <id> -t 5 -n 5
 *
 * 其余流程与 tv-indicator.js 完全一致：
 ...
 *
 * 用法：
 *   node clients/tv-indicator-kline.js -k <api_key> -i <tv_data_id> [-t 1] [-n 3]
 *   node clients/tv-indicator-kline.js              （全部从 .env 读取）
 */
...
// ============ 辅助K线默认配置（命令行未传参时使用；.env 为多客户端共用，故不读取） ============
const DEFAULT_KLINE_TIMEFRAME = '1'; // 辅助K线周期（分钟），写入键名 kline_<tf>m，命令行 -t 可覆盖
const DEFAULT_KLINE_COUNT = 3;       // 各周期写入的已收盘K线根数，命令行 -n 可覆盖

let currentClient = null;
let currentChart = null;
let currentAuxChart = null; // 辅助周期K线 chart（如 1m）
...
function parseArgs() {
  ...
  const config = {
    ...
    klineTimeframe: process.env.KLINE_TIMEFRAME || '1', // 辅助K线周期（分钟）
    klineCount: Number(process.env.KLINE_COUNT) || 3,   // 写入的已收盘K线根数
    debug: false,
  };
  args.forEach((arg, index) => {
    ...
    } else if (arg === '-t' || arg === '--kline-timeframe') {
      if (next && !next.startsWith('-')) config.klineTimeframe = next;
    } else if (arg === '-n' || arg === '--kline-count') {
      if (next && !next.startsWith('-')) config.klineCount = Number(next);
...
function printUsage() {
  console.log("TradingView 指标数据采集客户端（K线增强版：额外输出 cur_kline 与辅助周期K线）");
  console.log("");
  console.log("用法:");
  console.log("  node clients/tv-indicator-kline.js -k <api_key> -i <tv_data_id> [-t 1] [-n 3]");
  console.log("  node clients/tv-indicator-kline.js              （全部从 .env 读取）");
  console.log("");
  ...
  console.log("  -t, --kline-timeframe 辅助K线周期（分钟, 默认: 文件开头 DEFAULT_KLINE_TIMEFRAME=1，键名 kline_<tf>m）");
  console.log("  -n, --kline-count     各周期写入的已收盘K线根数（默认: 文件开头 DEFAULT_KLINE_COUNT=3）");
  ...
  console.log("  （辅助K线的周期/根数不读 .env，默认值见文件开头常量，命令行 -t/-n 覆盖）");
  console.log("");
  console.log("示例:");
  console.log("  node clients/tv-indicator-kline.js -k abc123def456 -i 1");
  console.log("  node clients/tv-indicator-kline.js -k abc123def456 -i 1 -t 1 -n 5");
}
...
/**
 * 从 chart.periods 提取已收盘K线（旧→新数组）
 * @param chart     TradingView Chart 会话（主 chart 或辅助 chart）
 * @param count     需要的已收盘K线根数
 * @param precision 价格保留小数位数
 * chart.periods 结构为 { time, open, close, max, min } 且按时间新→旧排序：
 *   periods[0] 为当前正在形成的未收盘K线，已收盘的从 periods[1] 开始
 * 字段映射：max → high，min → low
 * 注意：主 chart 若为 HeikinAshi，cur_kline 即为平均K线值（与指标所见一致）
 */
function extractKlines(chart, count, precision) {
  const p = precision || 2;
  if (!chart || !chart.periods || chart.periods.length === 0) return [];
  return chart.periods
    .slice(1, 1 + count) // 跳过 periods[0]（未收盘），只取已收盘
    .reverse()           // 旧→新
    .map((k) => ({
      time: k.time, // TV 原生秒级时间戳（K线开盘时间）
      open: Number(k.open.toFixed(p)),
      high: Number(k.max.toFixed(p)),
      low: Number(k.min.toFixed(p)),
      close: Number(k.close.toFixed(p)),
      volume: k.volume, // 库已对 volume 四舍五入到 2 位小数
    }));
}

function printKlines(label, klines) {
  if (!klines || klines.length === 0) {
    console.log(`\n--- ${label}: 无数据 ---`);
    return;
  }
  console.log(`\n--- ${label}（${klines.length}根已收盘）---`);
  klines.forEach((k) => {
    const t = new Date(k.time * 1000).toLocaleTimeString('zh-CN', { hour12: false });
    console.log(`  [${t}] O:${k.open} H:${k.high} L:${k.low} C:${k.close} V:${k.volume}`);
  });
}
...
function cleanupResources() {
  ...
  if (currentAuxChart) {
    try { currentAuxChart.delete(); } catch (e) {}
    currentAuxChart = null;
  }
  if (currentChart) {
...
async function initClient(apiConfig, tvData, tvCredentials) {
  ...
  console.log("模式: K线收盘时自动输出数据并写入开放接口（含 cur_kline / 辅助周期K线）");
  ...
  try {
    ...
    // ===== 辅助周期K线订阅（同一 WebSocket client，额外一条 chart session）=====
    // cur_kline 直接复用主 chart 的 periods，不额外开订阅；
    // 仅当辅助周期与主周期不同时才多开一条（如 1m）
    const auxTf = String(apiConfig.klineTimeframe || '1');
    if (auxTf === timeframe) {
      console.log(`辅助K线周期与主周期相同(${timeframe}分钟)，跳过额外订阅`);
    } else {
      const auxRange = (apiConfig.klineCount || 3) + 1; // 多取1根 forming K线
      currentAuxChart = new currentClient.Session.Chart();
      currentAuxChart.setMarket(market, { timeframe: auxTf, range: auxRange });
      console.log(`辅助K线订阅: ${auxTf}分钟 (range=${auxRange})`);
    }
...
async function handleKlineClose(apiConfig, tvDataId, closedPeriod, price) {
  ...
  // 附加K线数据（均为已收盘K线，旧→新）：
  //   cur_kline    → 主周期（当前订阅周期），复用主 chart periods
  //   kline_<tf>m  → 辅助周期（默认 1 分钟），来自额外的 chart 订阅
  trendData.cur_kline = extractKlines(currentChart, apiConfig.klineCount, precision);
  printKlines('cur_kline(主周期)', trendData.cur_kline);
  if (currentAuxChart) {
    const auxKey = `kline_${apiConfig.klineTimeframe}m`;
    trendData[auxKey] = extractKlines(currentAuxChart, apiConfig.klineCount, precision);
    printKlines(auxKey + '(辅助周期)', trendData[auxKey]);
  }
  ...
  // 打印最终提交给开放平台的完整数据
  // data 字段实际提交的是 JSON 字符串，此处解析展示便于阅读
  console.log("\n=== 提交给开放平台的最终数据 ===");
  console.log(JSON.stringify({ ...payload, data: trendData }, null, 2));
  console.log("================================\n");
